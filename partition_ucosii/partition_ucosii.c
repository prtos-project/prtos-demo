...
void TimerKicker() {
    ...
    InstallTrapHandler(XAL_PRTOSEXT_TRAP(PRTOS_VT_EXT_HW_TIMER), HwTimerHandler); /* Install timer handler */
    ...
    prtos_set_timer(PRTOS_HW_CLOCK, hwClock + 1000000LL, 1000000LL); /* Set hardware time driven timer */
}
...
static void AppTask1(void *pdata) {
    ...
    pdata = pdata;
    while (TRUE)
    { /* Task body, always written as an infinite */
